...
- 답변은 음성 활동을 자동으로 감지해 실시간 음성인식(STT)으로 변환되어 화면에 기록되며, 답변 후 10초간 침묵이 지속되면 다음 질문으로 자동으로 넘어갑니다.
- 면접 중 유저의 표정은 1초 간격으로 분석되어 자신감·긴장도·안정도 수치와 감정 변화 흐름으로 시각화됩니다.
...
- 면접 중지 버튼을 누르거나, AI 면접관이 모든 질문을 마치면 자동으로 면접이 종료되며 AI 종합 분석 리포트가 제공됩니다.
